...
const DefinitionBox = ({ children }) => (
    <div className="my-8 p-6 terminal-border bg-[var(--bg-secondary)] animate-fadeIn">
        <h3 className="font-bold text-xl mb-2 flex items-center gap-3">
            <BookMarked className="w-6 h-6 text-[var(--highlight-primary)]" />
            Definition
        </h3>
        <blockquote className="border-l-4 border-[var(--highlight-primary)] pl-4 text-[var(--text-secondary)] text-lg">
            {children}
        </blockquote>
    </div>
);
...
const ComplexityBox = ({ time, space, recurrence, theorem }) => (
    <div className="my-8 terminal-border p-6 bg-[var(--bg-secondary)] animate-fadeIn">
        ...
        {theorem && (
             <div className="mt-4">
                 ...
                 <p className="text-lg mt-2">{theorem}</p>
            </div>
        )}
    </div>
);
    ...
    <div className="my-6 p-4 animate-fadeIn">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
                <button onClick={onReset} className="p-2 terminal-border bg-[var(--bg-secondary)] hover:bg-[var(--border-color)] transition-colors" title="Reset"><RotateCcw size={20} /></button>
                <button onClick={onStepBack} disabled={currentStep === 0} className="p-2 terminal-border bg-[var(--bg-secondary)] hover:bg-[var(--border-color)] transition-colors disabled:opacity-50" title="Previous Step"><SkipBack size={20} /></button>
                <button onClick={onPlayPause} className="px-4 py-2 bg-[var(--success-color)] text-black terminal-border flex items-center gap-2 hover:opacity-80 transition-opacity disabled:opacity-50 font-bold" disabled={isFinished}>
                    {isPlaying ? <Pause size={20} /> : <Play size={20} />}
                    <span>{isPlaying ? 'PAUSE' : 'PLAY'}</span>
                </button>
                <button onClick={onStepForward} disabled={isFinished} className="p-2 terminal-border bg-[var(--bg-secondary)] hover:bg-[var(--border-color)] transition-colors disabled:opacity-50" title="Next Step"><SkipForward size={20} /></button>
            </div>
...
const TopicPage = ({ topic, onBackToHome }) => {
    ...
    return (
        <div className="flex w-full">
            <aside className={`bg-var(--bg-secondary) border-r border-var(--border-color) flex-shrink-0 transition-all duration-300 ${isSidebarOpen ? 'w-72' : 'w-0'} overflow-hidden`}>
            ...
            <div className="flex-1 relative">
                <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="fixed top-4 left-4 z-20 p-2 terminal-border bg-var(--bg-primary) hover:bg-var(--bg-secondary) transition-colors">
